...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:template name="htmlHead">
        <head>
            <title>
                <xsl:value-of select="testng:getVariableSafe($testNgXslt.reportTitle, 'OnGuard-Test Results')"/>
            </title>
    ...
    <xsl:template match="/testng-results">
        ...
        <xsl:result-document href="{testng:absolutePath('overview.html')}" format="xhtml">
            <html xmlns="http://www.w3.org/1999/xhtml">
                <xsl:call-template name="htmlHead"/>
				 <xsl:variable name="vToday" select="format-dateTime(current-dateTime(), '[h1]:[m01] [P] on [MNn] [D],[Y].')"/>

                <body>
                    <h2>OnGuard Batch Application Manager -3.14 Test suites overview</h2>
					Executed on: <xsl:sequence select="$vToday"/>
					 <table width="100%">
                        <tr>
        ...
        <xsl:result-document href="{testng:absolutePath('navigation.html')}" format="xhtml">
            <html xmlns="http://www.w3.org/1999/xhtml">
                ...
                <body>
					...
					<img width="30" height="30" src="./Images/onguard.png"/> 
                        <xsl:value-of select="testng:getVariableSafe($testNgXslt.reportTitle, ' OnGuard Batch Application Manager')"/>
                    </h2>
                    <div>
        ...
        <div id="testMethodsByStatus">
            ...
            <table class="testMethodsTable" cellpadding="0" cellspacing="0">
                <tr class="methodsTableHeader">
                    ...
                    <td>Exception</td>
                    <td>ScreenShots</td> <!-- Added new tag for screen shots-->
                </tr>
        ...
        <xsl:for-each select="$methodList">
            ...
            <tr id="{concat($methodId, '_row')}" class="{testng:testMethodStatus(.)}">
                ...
                <td nowrap="true"> <!-- Added new tag for screen shots-->
                    <xsl:if test="./exception">
                    	<xsl:variable name="methodName" select="concat(@name, '(', testng:trim(testng:concatParams(./params/param)), ')')"/>
                       <a href="./Screenshot/shot_{$methodName}.jpg" target="_blank">ScreenShot</a>
                    </xsl:if>
                    &#160;
                </td>
            </tr>
	...
	<div style="margin-top: 15px; color: blue; text-align: right; font-size: 14px;">
	    				
            Generated by:<b><br>Selenium Webdriver</br>Sumer Singh<br>Sr.QA Automation Engineer</br></b>
			<img width="150" height="40" src="./Images/tshirt-logo.png"/>
