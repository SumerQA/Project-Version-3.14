...
import Configration_Files.VideoReord;
...
public class User_Login extends TestListenerAdapter {	
	...
	@Parameters({"Locale"})
	public void Check_for_Valid_Username_Password(String Locale) throws Exception{
		...
		System.out.println("********TestCase- APP-36 :Check for valid username and password ********");
		Reporter.log("********TestCase- APP-36 :Check for valid username and password ********");
	...
	public void Check_Usergroups_in_Usergroup_combo_box() throws Exception{
		...
		OG_Login2.Select_UserGroup_from_DDList(1);
	...
	public void Check_Next_Button_Functionality() throws Exception{
		WebDriver driver=WebDriverManager.getDriverInstance();
		vr.startRecording(Thread.currentThread().getStackTrace()[1].getMethodName());
		OnGuard_Application_Manager_Login_UserGroup OG_Login2= PageFactory.initElements(driver, OnGuard_Application_Manager_Login_UserGroup.class);
		OnGuard_Application_Manager_Login OG_Login1= PageFactory.initElements(driver, OnGuard_Application_Manager_Login.class);
		...
		OG_Login1.Enter_User_Name("edwin");	
		OG_Login1.Enter_Password("kim");
		Thread.sleep(500);
		OG_Login1.ClickOn_next_Button();
		Thread.sleep(1000);
	...
	@Test(priority=5)
	@Parameters({"Locale"})
	public void Check_Inactive_Users_functionality(String Locale) throws Exception{
		WebDriver driver=WebDriverManager.getDriverInstance();
		vr.startRecording(Thread.currentThread().getStackTrace()[1].getMethodName());
		OnGuard_Application_Manager_Login OG_Login1= PageFactory.initElements(driver, OnGuard_Application_Manager_Login.class);
		System.out.println("********TestCase- APP-41:Check Next button in second stage of login screen ********");
		Reporter.log("********TestCase- APP-41:Check Next button in second stage of login screen ********>");
		Thread.sleep(1500);
		OG_Login1.Enter_User_Name("parts");	
		Thread.sleep(500);
		switch(Locale){
		 case("EN") :  OG_Login1.Choose_Language("English");
		 break;
	...
	@Test(priority=6)
	public void Check_Expired_Users_functionality() throws Exception{
		WebDriver driver=WebDriverManager.getDriverInstance();
		vr.startRecording(Thread.currentThread().getStackTrace()[1].getMethodName());
		OnGuard_Application_Manager_Login OG_Login1= PageFactory.initElements(driver, OnGuard_Application_Manager_Login.class);
		OnGuard_Application_Manager_Login_UserGroup OG_Login2= PageFactory.initElements(driver, OnGuard_Application_Manager_Login_UserGroup.class);
		...
		try {
			if(driver.findElement(By.xpath(c.getElementName("Confirm_Password"))).isDisplayed()){
				...
				OG_Login1.isNotificationPresent();
				Thread.sleep(500);
				OG_Login1.Old_password("kim1");
				Thread.sleep(500);
				OG_Login1.New_password("kim1");
				Thread.sleep(500);
				OG_Login1.Confirm_password("kim1");
				Thread.sleep(500);
				OG_Login1.ClickOn_next_Button(); 
				OG_Login1.isNotificationPresent();
				Thread.sleep(500);
				...
				OG_Login1.New_password("kim1");
				Thread.sleep(500);
				OG_Login1.Confirm_password("kim2");
				Thread.sleep(500);
				OG_Login1.ClickOn_next_Button();
				OG_Login1.isNotificationPresent();
				OG_Login1.Old_password("kim");
